...
public class SecondGamePlay extends CCColorLayer {		
	...
	//鲸鱼boy
	private List<GameSprite> fishs = new CopyOnWriteArrayList<GameSprite>();
	private String fishPath = "images/big_fish.png";
	private String fishSeqPath = "images/big_fish_seq5.png";
	private int fishLife = 8;
	private int fishDownSpeed = 13; // 下降的速度
	private String fish_Path_1 = "images/fish1.jpg";
	private String fish_Path_2 = "images/fish2.jpg"; 
	...
	protected SecondGamePlay(ccColor4B color) {
		super(color);
	...
	public void Detection(float t) {
		...
		for (int i = 0; i < qipaos.size(); i++) {
			...
			for (int j = 0; j < fishs.size(); j++) {
				GameSprite ff = fishs.get(j);
				CGRect rectff = ff.getCCSprite().getBoundingBox();
				if (CGRect.intersects(rectQipao, rectff)) {
					soundPool.play(musicId.get(2), 1, 1, 0, 0, 1);
					ff.Life -= 1;
					qipaos.remove(qipao);
					qipao.removeSelf();
					if (ff.Life <= 0) {
						...
						ChangeScore(500);
						// 碰到爱心之后需要分割					
						AddSpriteAnimal(ff.getCCSprite().getPosition(), fishSeqPath,
								ff.getCCSprite().getContentSize().width, ff.getCCSprite().getContentSize().height,
								3,0.3f);
						fishs.remove(ff);
						ff.getCCSprite().removeSelf();
					}else {
						ChageSpriteBack(ff, true, ff.getLife());
						ff.getCCSprite().removeSelf();
						fishs.remove(j);
					}				
				}
		...
		for (int i = 0; i < fishs.size(); i++) {
			GameSprite ff = fishs.get(i);
			CGRect rectff = ff.getCCSprite().getBoundingBox();
			if (CGRect.intersects(rectff, rectGirl)) {
			 soundPool.play(musicId.get(4), 1, 1, 0, 0, 1);
				fishs.remove(ff);
				ff.getCCSprite().removeSelf();
				girlLife -= 3;
	 ...
	 *击中更换鲸鱼图片
	 */
	private void ChageSpriteBack(GameSprite Foe, boolean Click, int life) {
		GameSprite _FeiJi_Foe = new GameSprite();
		_FeiJi_Foe.setClicked_Or(Click);
		_FeiJi_Foe.setLife(Foe.Life);

		switch (life){
		case 0:
			_FeiJi_Foe.setCCSprite(fish_Path_1);
			//changNum=1;
			break;
		case 1:
			_FeiJi_Foe.setCCSprite(fish_Path_2);
			//changNum=0;
			break;
		case 2:
			_FeiJi_Foe.setCCSprite(fish_Path_1);
		...
		float _sudu = (Foe.getInitY() + _FeiJi_Foe.getCCSprite().getContentSize().height / 2.0f)/ (float) Foe.getInitDuration();
		float time = ((Foe.getCCSprite().getPosition().y + _FeiJi_Foe.getCCSprite().getContentSize().height / 2.0f) / _sudu);
		_FeiJi_Foe.setInitDuration(time);
		Down(_FeiJi_Foe,"fish", Foe.getCCSprite().getPosition().y);
	 ...
	 * 给予时间和位置，添加敌机落下
	 */
	private void Down(GameSprite _FeiJi_Foe,String name, float y) {
		...
		if ("fish".equals(name))
			fs_Over = CCCallFuncN.action(this, "fishOverF");
	...
	public void AddSmallLove() {
		...
		GameSprite Love = new GameSprite();
		 ...
		 if(randomValue <=5 && fishs.size() < 1)	{    //保证鲸鱼在屏幕中只有一个
			Love.setCCSprite(fishPath);
				Love.setLife(fishLife);
				Love.setMaxLife(fishLife);
				SmallLoveDown(Love, "Fish",fishDownSpeed);
		 }else{
	...
	public void fishOverF(Object fish){
		CCSprite ff = (CCSprite) fish;
		ff.removeSelf();

		for (int i = 0; i <fishs.size(); i++) {
			GameSprite foe =fishs.get(i);
			if (foe.getCCSprite() == ff) {
				fishs.remove(i);
				break;
			}
		}
